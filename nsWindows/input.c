...
extern struct nsWindow g_window;
//extern struct s_window sWindow;
//extern struct nsDialog g_dialog;
//extern struct s_dialog sDialog;
//extern int i_windows;
...
static int NSDFUNC ConvertPlacement(char *str, int total, int height)
{
  ...
  else if (unit == 'u')
  {
    ...
    MapDialogRect(g_window.hwParent, &r);
...
int NSDFUNC PopPlacement(int *x, int *y, int *width, int *height)
{
  ...
  GetClientRect(g_window.hwWindow, &dialogRect);
